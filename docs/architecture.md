...
flowchart TB
  ...
  TM["TaskManager<br/>SQLite 任务表 + 任务事件"]
...
| 任务表 | `taskManager.ts` / `sqlite.ts` | SQLite 存任务与 `opencode_trace`；**对外事件只给 HTTP** |
...
flowchart LR
  subgraph 内部["给执行器用"]
    ...
    EB --> SR["SearchRunner"]
    EB --> TM0["TaskManager<br/>opencode_trace 列"]
  end
  subgraph 对外["给客户端用"]
...
| 消费者 | SearchRunner（进度/终态）+ persist fiber（写入 `opencode_trace`） | **只有 HTTP**（`syncResponse` / `sseResponse`） |
...
1. 路由 `TaskManager.create` → SQLite 里多一条 `queued` 任务。
...
`GET /api/search/:id` 不订事件，直接读表。任务在 SQLite（`SQLITE_PATH`，默认 `./data/search-agent.sqlite`），重启后仍可查。每次检索的原始 opencode RPC 与 SSE 写在同一行的 `opencode_trace` 列，**不进 HTTP 任务 JSON**。

`progress_json` 和 `opencode_trace` 不是同一份数据：

| 列 | 内容 | 谁读 |
|---|---|---|
| `progress_json` | `Task.progress`：中文阶段/工具摘要（给 SSE 和 `GET /api/search/:id`） | HTTP |
| `opencode_trace` | 原始 `session.*` 调用 + opencode SSE 事件 | 只在库里，用 sqlite 查看 |
...
SqliteLive             需要 AppConfig（打开库、migration）
OpenCodeLive           需要 AppConfig
OpenCodeOpsLive        需要 OpenCode + AppConfig + TaskManager（RPC 写入 opencode_trace）
TaskManagerLive        需要 AppConfig + SqlClient
EventBridgeLive        无依赖
...
```
src/index.ts                 入口：装配 + 路由 + 拉起 eventLoop / trace 落盘
src/env.ts                   AppConfig
src/domain/search.ts         请求 / 任务 / 结果 / OpencodeTrace Schema
