...
import {
  ...
  resolveSearchResult,
  type OpenCodeOpsService,
} from "./opencode.js"
import { waitSessionSettled } from "./sessionWait.js"
import { TaskManager, type TaskManagerService } from "./taskManager.js"
...
export type SearchRunnerService = Context.Service.Shape<typeof SearchRunner>

export interface SearchRunnerDeps {
  readonly ops: OpenCodeOpsService
  readonly tasks: TaskManagerService
  readonly bridge: EventBridgeService
  readonly config: { readonly taskTimeout: Duration.Duration }
}

/** 编排实现。Layer 只负责取出依赖;测试可传入假 ops / 任务表。 */
export function makeSearchRunner(deps: SearchRunnerDeps): SearchRunnerService {
  const { ops, tasks, bridge, config } = deps
...
export const SearchRunnerLive: Layer.Layer<
  SearchRunner,
  never,
  OpenCodeOps | TaskManager | EventBridge | AppConfig
> = Layer.effect(SearchRunner)(
  Effect.gen(function* () {
    return makeSearchRunner({
      ops: yield* OpenCodeOps,
      tasks: yield* TaskManager,
      bridge: yield* EventBridge,
